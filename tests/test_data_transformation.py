...
from src.components.data_transformation import add_input_features, NUM_FEATURES



def make_district_history(n_days: int = 20) -> pd.DataFrame:
    '''
    A small, synthetic 30 day history for one district - enough to fill every rolling window (longest is 7 days) with room to spare
    '''
...
def test_add_input_features_creates_expected_columns():
    ...
    for col in ["rain_lag1", "rain_lag2", "rain_lag3", "rain_3d", "rain_7d",
                "rain_max_7d", "wet_days_7d", "soil_change_3d", "soil_mean_7d",
                "month_sin", "month_cos"]:
